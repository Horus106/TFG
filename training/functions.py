...
def image_generator2(file_list, base_path, batch_size, datagen, target_size, colormode="rgb", shuffle=False, weights=False):
    import numpy as np
    ...
    while True:
        ...
        for i in range(len(file_list)):
            img_path = os.path.join(base_path, file_list[i][0])
            img = load_img(img_path, target_size=target_size, color_mode=colormode)
