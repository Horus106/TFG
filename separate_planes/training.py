...
# Redirigir cachés manualmente antes de cargar TensorFlow/Keras
os.environ['TMPDIR'] = '/mnt/homeGPU/jarodriguez/tmp'
os.environ['KERAS_HOME'] = '/mnt/homeGPU/jarodriguez/tmp/keras'
os.environ['TFHUB_CACHE_DIR'] = '/mnt/homeGPU/jarodriguez/tmp/tfhub'
...
def main(train_path, validation_path, test_path, data_path, checkpoint_filepath, 
    ...
    #     x = Average()([cnn_0.output, cnn_1.output, cnn_2.output])
    if resnet:
        print('FINE TUNNING (single input)')
        cnn_ft = fn.create_CNN_Resnet(0, img_shape[0], img_shape[1], depth, trainable=True)
        model_ft = Model(inputs=cnn_ft.input, outputs=cnn_ft.output)
    ...
    test = fn.get_images_list(test_df, color_mode, view, augment=True, weights=False)
    ...
    true = np.array([float(n[1]) for n in test])
    ...
    if not resnet:
        ...
        rmse.append(mean_squared_error(true, pred, squared=False) ** 0.5)
        mae.append(mean_absolute_error(true, pred))
